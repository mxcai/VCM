...
Once the parameter estimates $\hsigb$, $\hsige$ and $\hbfomega$ are obtained, we can recover the posterior mean by$$\bfmu=\left(\bfX^T\bfX+\frac{\hsige}{\hsigb}\bfI_p\right)^{-1}\bfX^T(\bfy-\bfZ\hbfomega)=\bfX^T\left(\bfX\bfX^T+\frac{\hsige}{\hsigb}\bfI_{n_1}\right)^{-1}(\bfy-\bfZ\hbfomega)$$. Note that because $\bfX$ has been standardized, we need to re-scale the posterior mean by $\bfmu\leftarrow\bfmu/\bfs/\sqrt{p}$.
...
if $n>p$, or $$\bfX^T\left(\bfX\bfX^T+\frac{\hsige}{\hsigb}\bfI_{n_1}\right)^{-1}(\bfy-\bfZ\hbfomega)$$ if $n<p$. Note that because $\bfX$ has been standardized, we need to re-scale the posterior mean by $\bfmu\leftarrow\bfmu/\bfs/\sqrt{p}$.
