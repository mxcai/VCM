...
\begin{split}
	...
	&= \tbfy^T\tbfy - 2\delta\tbfy^T\bfX\bfmu + \delta^2\tr(\bfX^T\bfX\bfSig),\\
...
\begin{split}
	...
	& - \frac{1}{2\sige}||\bfy-\bfZ\bfomega - \delta\bfX\bfmu||^2 - \frac{1}{2\sigb}||\bfmu||^2\\
	& - \frac{1}{2\sigb}\bfmu^T\bfmu - \tr\left(\left(\frac{\delta^2}{2\sige}\bfX^T\bfX + \frac{1}{2\sigb}\mathbf{I}_{p}\right)\bfSig\right).
\end{split}\nonumber
\end{align}
...
\UNTIL{the incomplete-data log-likelihood stop increasing or maximum iteration reached}
\end{algorithmic}
\end{algorithm}
...
\end{equation}
which separates the variance components in the quadratic term of the likelihood (5). Second, the convexity of function $-\log|\bfOmega|$ implies that
\begin{equation}
-\log|\bfOmegat| - \tr((\bfOmegat)^{-1}(\bfOmega-\bfOmegat))\leq-\frac{1}{2}\log|\bfOmega|,
\end{equation}
which separates the variance components in the log determinant of the likelihood (5). Combining (7) and (8), the overall minorization if given as
\begin{align}
\begin{split}
...
\end{split}\nonumber
\end{align}
The resulting algorithm is summarized in Algorithm 4.
...
\begin{algorithmic}
\STATE {Initialization: $\bfomega=(\bfZ^T\bfZ)^{-1}\bfZ^T\bfy$, $\sige=\sigb=\Var(y-\bfZ\bfomega)$}; $\bfK=\bfU\bfD\bfU^T$.
\REPEAT 
\STATE 
\begin{align}
\begin{split}
\td_i& = d_i/\siget + 1/\sigbt,\ \mathrm{diag}(\tbfD)=\td=[\td_i,...\td_n]\\
\bfomegat &= (\bfZ^T\tbfD\bfZ)^{-1}\bfZ^T(\bfy\odot\td),\\
(\sigma_{\beta}^{(t+1)})^2 &= (\sigma_{\beta}^{(t)})^2\sqrt{\frac{\sum_i^n[(\bfy_i-\bfZ_i^T\bfomegat)^2\td_i^2d_i]}{\sum_i^n\td_id_i}},\\
(\sigma_e^{(t+1)})^2 &= (\sigma_e^{(t+1)})^2\sqrt{\frac{\sum_i^n[(\bfy_i-\bfZ_i^T\bfomegat)^2\td_i^2]}{\sum_i^n\td_i}}.
\end{split}\nonumber
\end{align}
\UNTIL{the incomplete-data log-likelihood stop increasing or maximum iteration reached}
\end{algorithmic}
...
\end{equation}
. Using the fact that $||\mathbf{A}||_F=\sqrt{\tr(\mathbf{A}\mathbf{A}^T)}$, the OLS problem in (10) can be re-written as
\begin{equation*}
