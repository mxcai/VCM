...
cvperf.linReg <- function(X, y, nfolds=10, alg="PXEM", verbose=F, ...){
  p <- ncol(X)
  n <- nrow(X)
  ...
  for(i in 1:nfolds) {
    ...
    if(alg == "MoM"){
      fit <- linRegMoM(X_train,y_train,verbose=verbose,...)
    } else if(alg == "EM"){
      fit <- linRegMM(X_train,y_train,verbose=verbose,...)
    } else if(alg == "PXEM"){
      fit <- linRegPXEM(X_train,y_train,verbose=verbose,...)
    }


    testErr[i]  <- mean((y_test-predict(fit,X_test))^2)
